...
- `SOLAR_FORECAST_VERIFICATION_INTERVAL` (default `15m`; when `> 0`, the telemetry-mode gRPC runtime runs the background solar forecast verification loop against matured issued hourly rows, and when Valkey lease coordination is configured only one replica actively owns that loop at a time)
