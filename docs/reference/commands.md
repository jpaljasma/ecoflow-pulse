...
- `make dev-up-cloud-db` brings up the local platform and public edge, then
  ...
  NATS/Valkey. `make dev-up` is the full local reset path.
- `make dev-web-deploy-cloud-realtime` refreshes only the local public app and
  realtime gateway with the generated cloud-realtime overlay. Use it when the
  local web/auth shell is already up and cloud remains the telemetry data plane.
