...
- `make dev-up`:
  ...
  2) `helm dependency update deploy/charts/pulse-platform`
  3) `helm upgrade --install pulse-platform deploy/charts/pulse-platform -f deploy/env/local/values.platform.yaml`
  4) `helm dependency update deploy/charts/pulse-services`
  5) `helm upgrade --install pulse-services deploy/charts/pulse-services -f deploy/env/local/values.services.yaml`
- `make dev-down`:
