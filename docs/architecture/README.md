...
| TODO | Local platform cold-start recovery hardening<br>- [ ] Detect and surface k3d node readiness failures before platform waits claim success<br>- [ ] Strengthen local dependency gates for CNPG, NATS, Valkey, and Keycloak readiness after Docker/Kubernetes restarts<br>- [ ] Add startup retry/backoff for dependency-bound services so cold starts recover without widespread CrashLoopBackOff<br>- [ ] Add a documented local recovery command/path for stranded stateful pods after host restarts<br>- [ ] Capture acceptance evidence from a full cold-start recovery drill | M0 platform baseline |
