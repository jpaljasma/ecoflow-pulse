...
| PROGRESS | Load tests (k6): ingest + WS + query<br>- [x] Mark task `PROGRESS`, confirm dependency `M4` is complete, and capture kickoff scope (ingest publish, WS fanout, query read path)<br>- [ ] Add k6 scenarios + shared fixtures for ingest, websocket, and query coverage<br>- [ ] Wire reproducible local command(s) + docs for load execution and thresholds<br>- [ ] Record validation evidence, close checklist, and mark task `DONE`<br>- [ ] Validation: `make test-load-k6` | M4 |
