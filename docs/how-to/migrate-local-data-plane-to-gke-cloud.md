...
- GCS bucket exists for the raw archive:
  ...
  - prefix: `raw`
- GCS bucket lifecycle is applied from
  `deploy/env/cloud/archive-object-lifecycle.json`
  and bucket soft delete is disabled (`0`) so the effective hosted raw-object
  retention stays at the locked `30 days`
...
Before mirroring any data, apply the checked-in lifecycle contract to the cloud
bucket so hosted retention matches the architecture spec:

```bash
gcloud storage buckets update gs://pulse-telemetry-raw \
  --project=ecoflow-pulse-dev-260221-01 \
  --lifecycle-file=deploy/env/cloud/archive-object-lifecycle.json

gcloud storage buckets update gs://pulse-telemetry-raw \
  --project=ecoflow-pulse-dev-260221-01 \
  --soft-delete-duration=0

gsutil lifecycle get gs://pulse-telemetry-raw
gcloud storage buckets describe gs://pulse-telemetry-raw \
  --project=ecoflow-pulse-dev-260221-01 \
  --format='json(lifecycle_config,soft_delete_policy)'
```

The lifecycle JSON deletes only `raw/` objects older than `30 days`. Disabling
soft delete (`0`) prevents GCS from keeping an extra hidden recovery tail beyond the
locked raw-archive retention window.
