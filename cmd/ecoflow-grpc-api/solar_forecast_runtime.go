...
func newSolarVerificationLeaseManagerFromEnv(log *slog.Logger) (solarVerificationLeaseManager, func(), string, error) {
	...
	if err != nil {
		return nil, nil, "", fmt.Errorf("create solar forecast verification valkey client: %w", err)
	}
	manager, err := ingestlease.NewManager(client, ingestlease.DefaultConfig())
	if err != nil {
		client.Close()
		return nil, nil, "", fmt.Errorf("create solar forecast verification lease manager: %w", err)
	}
...
func solarVerificationWorkerID() string {
	...
	return fmt.Sprintf("grpc-api-%d", os.Getpid())
}
