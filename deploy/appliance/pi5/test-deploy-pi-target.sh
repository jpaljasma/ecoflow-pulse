...
require_pattern 'WAIT_TIMEOUT="\$\(PI_APPLIANCE_WAIT_TIMEOUT\)" APPLIANCE_PI_INSTALL_ARGS="--release-values \$\(PI_APPLIANCE_RELEASE_VALUES\) --skip-host-prepare --skip-k3s-install" \$\(MAKE\) appliance-pi-upgrade' "appliance upgrade invocation"
