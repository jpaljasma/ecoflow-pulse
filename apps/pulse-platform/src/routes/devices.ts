...
export function registerDeviceRoutes(
  ...
  app.post('/api/v1/devices/available/import', { preHandler: authPreHandler }, async (request, reply) => {
    try {
      const body = z.object({
        provider: z.string().trim().min(1),
        credentialId: z.string().trim().min(1),
        ...
        ingestDesiredState: z.string().trim().min(1).optional()
      }).parse(request.body);
      const result = await deviceClient.importAvailableDevice(request, body);
      return result;
    } catch (error) {
      if (isMissingUserSubjectError(error)) {
        return reply.code(503).send({
