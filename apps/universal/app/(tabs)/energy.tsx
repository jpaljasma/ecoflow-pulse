...
function findPVHistoryRow(rows: EnergyPVPortHistory[], deviceId: string, portId: string): EnergyPVPortHistory | undefined {
  ...
  return rows.find((row) => row.deviceId === deviceId && canonicalPvPortKey(row.portId) === expectedKey);
}
...
export default function EnergyScreen() {
  ...
  const selectedDateIso = useMemo(
    () => routeState.date ?? getTimezoneDateIso(new Date(), routeState.timezone),
    [routeState.date, routeState.timezone]
  );
  ...
  const previousEnergyNet = useMemo(
    () =>
      previousEnergySeries.solar.map(
        (_, idx) =>
  ...
  const weatherErrorText =
    describeQueryError(profileWeather.forecastQuery.error) ?? describeQueryError(profileWeather.solarOutlookQuery.error);
  const weatherEnabled = authReady && allowed && resolvedWeatherState.enabled;
  const weatherIsLoading =
    routeState.panel === 'solar' && (profileWeather.forecastQuery.isLoading || profileWeather.solarOutlookQuery.isLoading);
  ...
  const pvCardWidth =
    width >= 1500 ? Math.max(320, Math.floor((width - 116) / 3)) : width >= 980 ? Math.max(320, Math.floor((width - 92) / 2)) : undefined;
  ...
  const topCardTitle =
    routeState.panel === 'solar' ? 'Solar forecast' : routeState.panel === 'impact' ? 'Energy impact' : 'Solar against load';
    ...
    <Animated.View style={{ flex: 1 }} testID="screen-energy">
      <YStack flex={1} backgroundColor="$background">
        <TopBar
          ...
          right={
            <AppMenu weatherScope={routeState.scope} weatherDeviceId={routeState.scope === 'device' ? routeState.deviceId : undefined} />
          }
        />
        ...
        >
          ...
          >
            <XStack justifyContent="space-between" alignItems="flex-start" gap="$3" flexWrap="wrap">
              <YStack gap="$2" flex={1} minWidth={260}>
                ...
                <Text color="$colorMuted">{topCardSubtitle}</Text>
              </YStack>
              <Button
                size="$3"
                circular
                ...
                <YStack gap="$3">
                  ...
                  <XStack gap="$2" flexWrap="wrap" alignItems="center">
                    <Button
                      size="$3"
                      borderWidth={1}
                      style={buttonStyles(false, semantics)}
                      onPress={() =>
                        updateRoute({
                          preset: 'today',
                          date: shiftEnergyDateIso(selectedDateIso, -1)
                        })
                      }
                      accessibilityLabel="Previous day"
                    >
                      <MaterialCommunityIcons name="chevron-left" size={18} color={semantics.periodIdleText} />
                    </Button>
                    <Input
                      size="$3"
                      width={154}
                      value={selectedDateIso}
                      testID="energy-date-picker"
                      aria-label="Energy date"
                      {...(Platform.OS === 'web'
                        ? ({
                            type: 'date',
                            max: todayDateIso
                        ...
                        : {})}
                      onChangeText={(value) => {
                        if (/^\d{4}-\d{2}-\d{2}$/.test(value) && value <= todayDateIso) {
                          updateRoute({ preset: 'today', date: value });
                        }
                      }}
                      style={{
                        backgroundColor: semantics.periodIdleBackground,
                        borderColor: semantics.periodIdleBorder,
                        color: semantics.periodIdleText
                      }}
                    />
                    <Button
                      size="$3"
                      borderWidth={1}
                      style={buttonStyles(selectedDateIso === todayDateIso, semantics)}
                      onPress={() => updateRoute({ preset: 'today', date: todayDateIso })}
                    >
                      Today
                    </Button>
                    <Button
                      size="$3"
                      borderWidth={1}
                      disabled={selectedDateIso >= todayDateIso}
                      style={buttonStyles(false, semantics)}
                      onPress={() =>
                        updateRoute({
                          preset: 'today',
                    ...
                    >
                      <MaterialCommunityIcons name="chevron-right" size={18} color={semantics.periodIdleText} />
                    </Button>
                    <YStack
                      paddingHorizontal="$3"
                      paddingVertical="$2"
          ...
          {devicesQuery.isLoading && !devices.length ? <BrandedLoadingState minHeight={200} message="Loading energy scope…" /> : null}
          ...
          {routeState.panel === 'solar' ? (
            <YStack gap="$3">
                ...
                <XStack justifyContent="space-between" alignItems="flex-start" gap="$3" flexWrap="wrap">
                  <YStack gap="$1" flex={1} minWidth={240}>
                    <Text fontSize="$6" fontWeight="800">
                      {routeState.scope === 'all' ? 'Fleet solar forecast' : `${selectedDevice?.name ?? 'Device'} solar forecast`}
                    </Text>
                    <Text color="$colorMuted">
                      Weather-aware production outlook for the current Energy scope. Use Scope controls above to deep-link this pane for a
                      specific device.
                    </Text>
                  </YStack>
                  <YStack
              ...
              <XStack gap="$3" flexWrap={splitSectionWrap(isWideLayout)} flexDirection={isWideLayout ? 'row' : 'column'}>
            ...
            <YStack gap="$3">
              ...
              >
                ...
                <Text color="$colorMuted">
                  Measured solar generation only. No annualized estimates, no invented totals, just the currently selected fleet or device
                  scope translated into avoided-emissions summaries.
                </Text>
              </Card>
              {routeState.scope === 'all' ? (
            ...
            <>
                  ...
                  <XStack gap="$4" flexWrap="wrap" minWidth={260}>
                    <YStack gap="$1" minWidth={120}>
                      ...
                      <Text fontSize="$6" fontWeight="800" letterSpacing={-0.4}>
                        {formatCurrency(dashboardQuery.data.summary.estimatedValue.current, dashboardQuery.data.summary.currency)}
                      </Text>
                    </YStack>
                    <YStack gap="$1" minWidth={120}>
                      <Text fontSize="$2" style={{ color: semantics.subtleText }} textTransform="uppercase" letterSpacing={0.6}>
              ...
              <EnergyComparisonWidget data={comparisonInsightQuery.data} loading={comparisonInsightQuery.isLoading} />
              ...
              <XStack gap="$3" flexWrap={splitSectionWrap(isWideLayout)} flexDirection={isWideLayout ? 'row' : 'column'}>
                ...
                <YStack gap="$3" flex={1} minWidth={0}>
                  <ChartSection title="Energy balance" subtitle="Solar, grid, storage, and load over the selected local-calendar window.">
                    {dashboardQuery.data.currentEnergyPoints.length > 1 ? (
                      <YStack gap="$3">
                        ...
                        />
                        <XStack gap="$3" flexWrap="wrap">
                          <Stat label="Points" value={String(dashboardQuery.data.currentEnergyPoints.length)} compact />
                          {comparisonEnabled ? (
                            <Stat label="Previous" value={String(dashboardQuery.data.previousEnergyPoints.length)} compact tone="muted" />
                          ) : null}
                          <Stat
                      ...
                      <Text color="$colorMuted">Energy bucket history is not populated yet for this window.</Text>
                    )}
                  </ChartSection>
                  ...
                  <XStack gap="$3" flexWrap="wrap">
                    ...
                    <SectionCard title="AC input cost" minWidth={220}>
                      ...
                      {comparisonEnabled ? (
                        ...
                        <Text color="$colorMuted">Previous-period comparison stays available when comparison is enabled.</Text>
                      )}
                    </SectionCard>
                  </XStack>
                </YStack>
              ...
              <XStack gap="$3" flexWrap={splitSectionWrap(isWideLayout)} flexDirection={isWideLayout ? 'row' : 'column'}>
                <YStack flex={1} minWidth={0}>
                  <ChartSection title="Power profile" subtitle="Server-returned power buckets from the selected window.">
                    {dashboardQuery.data.currentPowerPoints.length > 1 ? (
                      ...
                      <Text color="$colorMuted">Power bucket history is not populated yet for this window.</Text>
                    )}
                  </ChartSection>
                </YStack>
              </XStack>
                  ...
                  routeState.scope === 'all' ? (
                    <XStack gap="$2" flexWrap="wrap">
                      {pvEnvelope.rows.map((row) => {
                        const historyRow = findPVHistoryRow(pvHistoryRows, row.deviceId, row.portId);
                        return (
                          <YStack
                          ...
                          >
                            ...
                            <XStack gap="$3" flexWrap="wrap">
                              ...
                              <Stat
                                label="Observed V/A"
                                value={`${row.observedVolts.toFixed(1)}V · ${row.observedAmps.toFixed(2)}A`}
                                compact
                              />
                              <Stat
                                label="Utilization"
                        ...
                        return (
                          ...
                          >
                            ...
                            <XStack gap="$3" flexWrap="wrap">
                              ...
                              <Stat
                                label="Headroom V"
                                value={row.voltageHeadroom === null ? '—' : `${row.voltageHeadroom.toFixed(1)}V`}
                                compact
                              />
                              <Stat
                                label="Headroom A"
                                value={row.currentHeadroom === null ? '—' : `${row.currentHeadroom.toFixed(1)}A`}
                                compact
                              />
                            </XStack>
                            {pvHistoryQuery.isLoading ? (
