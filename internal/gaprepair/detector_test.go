...
func TestDefaultDetectorConfigLagAlertDefaults(t *testing.T) {
	t.Parallel()

	cfg := DefaultDetectorConfig()
	if cfg.LagAlertWindow != 15*time.Minute {
		t.Fatalf("lag alert window mismatch: got=%s want=15m", cfg.LagAlertWindow)
	}
	if cfg.LagAlertThreshold != 12 {
		t.Fatalf("lag alert threshold mismatch: got=%d want=12", cfg.LagAlertThreshold)
	}
	if cfg.LagAlertCooldown != 5*time.Minute {
		t.Fatalf("lag alert cooldown mismatch: got=%s want=5m", cfg.LagAlertCooldown)
	}
}
