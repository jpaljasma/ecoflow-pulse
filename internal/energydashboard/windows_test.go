...
func TestResolveWindowLastSevenDaysUsesCalendarDayStart(t *testing.T) {
	...
	if got, want := window.From.Format(time.RFC3339), "2026-03-05T05:00:00Z"; got != want {
		t.Fatalf("from mismatch: got=%s want=%s", got, want)
	}
	if got, want := window.PreviousFrom.Format(time.RFC3339), "2026-02-26T05:00:00Z"; got != want {
		t.Fatalf("previous from mismatch: got=%s want=%s", got, want)
	}
	if got, want := window.PreviousTo.Format(time.RFC3339), "2026-03-05T05:00:00Z"; got != want {
		t.Fatalf("previous to mismatch: got=%s want=%s", got, want)
	}
}
