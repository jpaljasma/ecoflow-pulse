...
func TestPostgresStoreApplyEnvelopeIntegratesExplicitEnergyBucketsAcrossSparseMinuteBoundary(t *testing.T) {
	t.Parallel()
	...
	second := &envelopev1.TelemetryEnvelope{
		...
		EcoflowSn:          "DEMOD2M00001057",
		ObservedTimeUnixMs: time.Date(2026, time.March, 7, 12, 1, 15, 0, time.UTC).UnixMilli(),
		Payload:            []byte(`{"params":{"wattsInSum":240,"pv1ChargeWatts":40,"wattsOutSum":210,"f32ShowSoc":25.0}}`),
		Labels:             map[string]string{"provider": "ecoflow"},
	}
