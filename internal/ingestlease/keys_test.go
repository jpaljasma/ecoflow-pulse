...
func TestKeysForRef(t *testing.T) {
	keys, err := KeysForRef(LeaseRef{Provider: "ecoflow", ProviderDeviceID: "DEMOD2M00001057"})
	if err != nil {
		t.Fatalf("KeysForRef returned error: %v", err)
	}
	wantTag := "{ecoflow|DEMOD2M00001057}"
	if keys.HashTag != wantTag {
		t.Fatalf("HashTag mismatch: got=%q want=%q", keys.HashTag, wantTag)
	}
	if keys.Lease != "pulse:v1:ingest:lease:{ecoflow|DEMOD2M00001057}" {
		t.Fatalf("Lease key mismatch: %q", keys.Lease)
	}
	if keys.Session != "pulse:v1:ingest:session:{ecoflow|DEMOD2M00001057}" {
		t.Fatalf("Session key mismatch: %q", keys.Session)
	}
	if keys.Fence != "pulse:v1:ingest:fence:{ecoflow|DEMOD2M00001057}" {
